// Agent template schema + compiler. A template is a single editable prompt plus a small structured
// header the user tunes. The compiler validates it and turns it into a runnable AgentConfig + a
// templateHash (bytes32) that matches what AgentRegistry stores on-chain. Same template in -> same
// hash out. Lives in /shared so the agent runtime and the register UI compile identically.

import { keccak256, toBytes } from "viem";
import type { Bytes32, EvidenceSource, RiskAppetite, UsdcBaseUnits } from "./types.js";
